...
const { normalizeConfig } = require('./lib/simplifyconfig')
const { parseToken, splitTokens, interpretTokens } = require('./lib/parsers')
const { replicate, intersperse, splitWhen, log, copy } = require('./lib/utils')
...
// :: type Store s a = { get: s -> a, pos: s }
// :: type Stream = Store Int
// :: type Parser t e a = Stream t -> [Either (e, Stream t) (a, Stream t)]
function parse(args, opts) {
  if (!opts) opts = {}

  // Copy the input before stuff starts getting mutated
  const args_ = copy(args)
  const opts_ = normalizeConfig(copy(opts))
  let argv1
  {
    const input = typeof args_ === 'string' ? tokenizeArgString(args_) : args_
    log({ input })
    log({ opts })
    // log({ opts_ })

    const [rawTokens, tail] = splitWhen(x => x === '--')(input)
    const tokens = splitTokens(rawTokens.map(parseToken))
    argv1 = interpretTokens(opts_)(tokens)
    // log({ tokens, tail })
    log({ argv1 })
  }
  ...
  function maybeCoerceNumber(key, value) {
    if (
      ...
      !checkAllAliases(key, flags.coercions)
    ) {
  ...
  function setKey(obj, keys, value) {
    ...
    if (value === increment) {
      ...
      !(
        o[key] === undefined ||
        checkAllAliases(key, flags.bools) ||
        checkAllAliases(keys.join('.'), flags.bools) ||
        checkAllAliases(key, flags.counts)
      )
    ) {
      o[key] = [o[key], value]
